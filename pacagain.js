...
function movePlayer(ctx){
    ...
    if(playing){
        if(player.state == PLAYER_MOVING){
            ...
            var destX = player.x + DIRECTIONS[player.direction].dx * STEP / 4;
            var destY = player.y + DIRECTIONS[player.direction].dy * STEP / 4;            
    ...
    if(player.state == PLAYER_COMPLETED){
        playing = false;        
        player.aberturaBoca = 0;
        player.size *= 1.2;
        if(player.size > canvas.width/8){            
